 ...
 * 
 */
public class ZMQQueueServerImpl extends AbstractLifecycleComponent<ZMQServerTransport> implements ZMQServerTransport {

    private final String routerBinding;
    private int nbWorkers;
    private String workersBinding;
    ...
    protected ZMQQueueServerImpl(Settings settings, NodeService nodeService, ZMQRestImpl client, NetworkService networkService) {
        ...
        routerBinding = settings.get("zeromq.router.bind", "tcp://127.0.0.1:9300");
        ...
        logger.info("ØMQ settings [zeromq.router.bind={}, zeromq.workers.threads={}, zeromq.workers.bind={}]", routerBinding, nbWorkers,
            workersBinding);
    ...
    protected void doStart() throws ElasticsearchException {
        ...
        try {
            bindAddress =
                new InetSocketAddress(networkService.resolveBindHostAddress(ZMQAddressHelper.getHostName(workersBinding)),
                    ZMQAddressHelper.getPort(workersBinding));
        } catch (IOException e) {
            throw new BindHttpException("Failed to resolve host [" + workersBinding + "]", e);
        }
        waitForSocketsClose = new CountDownLatch(nbWorkers);
        for (int index = 0; index < nbWorkers; index++) {
            logger.debug("Creating worker #{}", index);
            ZMQSocket worker = new ZMQSocket(logger, context, workersBinding, index, client, isRunning, waitForSocketsClose);
            daemonThreadFactory(settings, "zeromq_worker_" + index).newThread(worker).start();
            sockets.add(worker);
        }
        ...
        try {
            publishAddress =
                new InetSocketAddress(networkService.resolvePublishHostAddress(ZMQAddressHelper.getHostName(routerBinding)),
                    ZMQAddressHelper.getPort(routerBinding));
        } catch (Exception e) {
