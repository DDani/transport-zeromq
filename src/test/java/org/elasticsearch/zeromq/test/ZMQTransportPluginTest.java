...
public class ZMQTransportPluginTest {
     ...
     */
    private static final String ADDRESS = "tcp://localhost:9700";

    @BeforeClass
    public static void setUpBeforeClass() throws Exception {
        // Instantiate an ES server
        setNode(NodeBuilder.nodeBuilder().settings(ImmutableSettings.settingsBuilder().put("es.config", "elasticsearch.yml")).node());
        // Instantiate a ZMQ context
        context = ZMQ.context(1);
    }


    // TODO : DANGER; VERY IMPORTANT This is failing, and can cause frezees on Elastic Search shutdown process
    @AfterClass
    ...
    private JSONResult sendAndReceive(String method, String uri, String json) throws JSONException {
        ...
        String result = null;
        try {
            socket.send(sb.toString().getBytes(UTF_8), 0);
            byte[] response = socket.recv(0);
            result = new String(response, Charset.forName(UTF_8));
        ...
        int firstSeparator = result.indexOf(SEPARATOR);
        int secondSeparator = result.indexOf(SEPARATOR, firstSeparator + 1);
    ...
    public static void setNode(Node node) {
        ZMQTransportPluginTest.node = node;
    }
